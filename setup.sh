...
proot-distro login ubuntuubuntu --shared-tmp -- env DISPLAY=:1.0 cp /usr/share/zoneinfo/$timezone /etc/localtime
}
...
if [[ $action == "Copy .desktop file" ]]; then
  selected_file=$(zenity --file-selection --title="Select .desktop File" --file-filter="*.desktop" --filename="../usr/var/lib/proot-distro/installed-rootfs/ubuntuubuntu/usr/share/applications")
  ...
  sed -i "s/^Exec=\(.*\)$/Exec=proot-distro login ubuntuubuntu --user $username --shared-tmp -- env DISPLAY=:1.0 \1/" "../usr/share/applications/$desktop_filename"
...
mkdir .fonts 
mkdir $HOME/../usr/var/lib/proot-distro/installed-rootfs/ubuntuubuntu/home/$username/.fonts/
...
sed -i '327s/termux/proot/' $HOME/../usr/var/lib/proot-distro/installed-rootfs/ubuntuubuntu/home/$username/.fancybash.sh
...
echo ""
echo "Bây giờ bạn có thể kết nối với Máy tính để bàn Termux XFCE4 để mở máy tính để bàn bằng lệnh bắt đầu"
echo ""
echo "Thao tác này sẽ khởi động máy chủ termux-x11 trong termux và khởi động XFCE Desktop, sau đó mở ứng dụng Termux-X11 đã cài đặt."
echo ""
echo "Để thoát, nhấp đúp vào biểu tượng Kill Termux X11 trên màn hình nền."
echo ""
